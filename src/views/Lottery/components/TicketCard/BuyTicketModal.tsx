...
const BuyTicketModal: React.FC<BuyTicketModalProps> = ({ max, onDismiss }) => {
  ...
  return (
    <Modal title={t('Enter amount of tickets to buy')} onDismiss={onDismiss}>
      ...
      />
      <div>
        <Tips>{t(`1 Ticket = ${LOTTERY_TICKET_PRICE} MAKI`, { num: LOTTERY_TICKET_PRICE })}</Tips>
      </div>
      <div>
        <Announce>
          {t('Ticket purchases are final. Your MAKI cannot be returned to you after buying tickets.')}
        </Announce>
        <Final>{t(`You will spend: ${cakeCosts(val)} MAKI`)}</Final>
      </div>
      <ModalActions>
