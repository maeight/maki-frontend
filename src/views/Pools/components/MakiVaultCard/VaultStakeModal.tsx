...
const VaultStakeModal: React.FC<VaultStakeModalProps> = ({ pool, stakingMax, isRemovingStake = false, onDismiss }) => {
  ...
  const usdValueStaked =
    makiPriceHusd.gt(0) && stakeAmount ? formatNumber(new BigNumber(stakeAmount).times(makiPriceHusd).toNumber()) : ''
  ...
  const handleWithdrawal = async (convertedStakeAmount: BigNumber) => {
    setPendingTx(true)
    const shareStakeToWithdraw = convertMakiToShares(convertedStakeAmount, pricePerFullShare)
    // trigger withdrawAll function if the withdrawal will leave 0.000001 MAKI or less
    const triggerWithdrawAllThreshold = new BigNumber(1000000000000)
    const sharesRemaining = userShares.minus(shareStakeToWithdraw.sharesAsBigNumber)
    const isWithdrawingAll = sharesRemaining.lte(triggerWithdrawAllThreshold)

    if (isWithdrawingAll) {
      makiVaultContract.methods
        .withdrawAll()
        .send({ from: account })
        .on('sending', () => {
          setPendingTx(true)
  ...
  return (
    <Modal
      ...
      onDismiss={onDismiss}
      headerBackground={theme.colors.gradients.bubblegum} // cardHeader -> Bubblegum
    >
      ...
      <BalanceInput
        ...
        currencyValue={makiPriceHusd.gt(0) && `~${usdValueStaked || 0} USD`}
