...
const Farm: React.FC = () => {
  ...
  const poolsWithApy = pools.map((pool) => {
    ...
    const totalRewardPricePerYear = rewardTokenPriceInHT.times(pool.tokenPerBlock).times(BLOCKS_PER_YEAR)
    const totalStakingTokenInPool = stakingTokenPriceInHT.times(getBalanceNumber(pool.totalStaked))
      ...
      <Hero>
        <div>
          <Heading as="h1" size="xxl" mb="16px">
            {TranslateString(738, 'Maki Pool')}
          </Heading>
          <ul>
            <li>{TranslateString(580, 'Stake MAKI to earn new tokens.')}</li>
            <li>{TranslateString(486, 'You can unstake at any time.')}</li>
            <li>{TranslateString(406, 'Rewards are calculated per block.')}</li>
          </ul>
        </div>
        <img src="/images/syrup.png" alt="Maki POOL icon" width={410} height={191} />
      </Hero>
