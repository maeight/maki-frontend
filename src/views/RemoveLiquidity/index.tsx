...
export default function RemoveLiquidity({
  ...
  async function onAttemptToApprove() {
    ...
    library
      .send('ht_signTypedData_v4', [account, data])
      .then(splitSignature)
      .then((signature) => {
        setSignatureData({
          v: signature.v,
  ...
  async function onRemove() {
    ...
    const currencyBIsHT = currencyB === HUOBI
    const oneCurrencyIsHT = currencyA === HUOBI || currencyBIsHT
  ...
  const oneCurrencyIsHT = currencyA === HUOBI || currencyB === HUOBI
  const oneCurrencyIsWHT = Boolean(
    chainId &&
      ((currencyA && currencyEquals(WHT[chainId], currencyA)) ||
        (currencyB && currencyEquals(WHT[chainId], currencyB)))
  )
                    ...
                    <AutoColumn gap="10px">
                      ...
                      {chainId && (oneCurrencyIsWHT || oneCurrencyIsHT) ? (
                        <RowBetween style={{ justifyContent: 'flex-end' }}>
                          {oneCurrencyIsHT ? (
                            <StyledInternalLink
                              to={`/remove/${currencyA === HUOBI ? WHT[chainId].address : currencyIdA}/${
                                currencyB === HUOBI ? WHT[chainId].address : currencyIdB
                              }`}
                            >
                              Receive WHT
                            </StyledInternalLink>
